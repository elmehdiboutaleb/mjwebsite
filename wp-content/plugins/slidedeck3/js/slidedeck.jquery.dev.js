...
jQuery(".lens-classic dl").hover(
		function() { 
			jQuery("dt.spine").fadeIn(); 
		},
		function() { 
			jQuery("dt.spine").fadeOut(); 
		}
	);
} );
    ...
    window.SlideDeck = function(el,opts){
		...
		var slidedeckAdjustImage = function( slide ){
			...
			iw = im[0].naturalWidth;
				...
				var nh = im.height(),
					...
					wd = (nw-tw)/2;
				if(nw<tw){
					im.addClass('slidedeck-adjust-width');
					wd = 0;
				}
			...
			if( ( self.options.auto_height || screen.width < 700 ) && self.options.hideSpines === true && vertical === false ) {
						...
						if(image.length !== 0){
							image.on('load', function() {
								if(slide.closest("#slidedeck-options-groups").length === 0){
									var slide_height = image[0].clientHeight;
									...
									if( preview_iframe !== null ){
										var nav_height = jQuery('.slidedeck-frame .nav-slidedeck').height();
        ...
        var buildDeck = function(){
            ...
            for(var i=0; i<self.slides.length; i++){
				...
				var preview_iframe = parent.document.getElementById('slidedeck-preview');
				if( preview_iframe !== null ){
						$(slide).children('img').addClass('slidedeck-adjust-width');
				}
        ...
        var transitions = {
            ...
            fade: function(ind, params, forward){
				...
				if( self.options.auto_height || self.options.device_width < 700 ) {
					...
					if( image.length > 0 ){
						var image_height = image[0].clientHeight;
						$currentSlide.css('height',image_height);
						$currentSlide.context.style.transition = "all 1s";
						$currentSlide.context.style.height = image_height + "px";
						...
						if( preview_iframe !== null ){
							var nav_height = jQuery('.slidedeck-frame .nav-slidedeck').height();
							preview_iframe.style.height = image_height + nav_height + 50 + "px";
							$($currentSlide).children('img').addClass('slidedeck-adjust-width');
						}
					}
				} else {
                ...
                for (var i = 0; i < self.slides.length; i++) {
					...
					if( self.options.auto_height || self.options.device_width < 700 ) {
						if( i === (self.current - 1) ){
							...
							if( image.length > 0 ){
								var image_height = image[0].clientHeight;
								self.slides.context.style.height = image_height + "px";
							}
						}
						slide.stop().animate({
							left: pos,
					...
					if(self.options.hideSpines === true){
						...
						if( self.options.auto_height || self.options.device_width < 700 ) {
							...
							if( slide.find('.slidedeck-vertical-center-inner').length > 0 ){
								...
								var new_height = ( parseInt(height) - nav_height - 10) + "px";
								slide.stop().animate({
									left: pos + "px",
									width: self.slide_width + "px",
									height: new_height
        ...
        this.loaded = function(func){
            loaded(func);
            autoAdjustHeight();
			if(self.options.hideSpines !== true && self.options.device_width < 700 ){
				autoAdjustHeight();
			}
            return self;
        };
