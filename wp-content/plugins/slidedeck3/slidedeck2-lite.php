<?php
/**
 * SlideDeck 3 Lite for WordPress - Slider Widget
 *
 * Create SlideDecks on your WordPress blogging platform. Manage SlideDeck
 * content and insert them into templates and posts.
 *
 * @package SlideDeck
 * @subpackage SlideDeck 3 Lite
 * @author SlideDeck
 */
/*
 Plugin Name: SlideDeck 3 Lite
 Plugin URI: http://demo.slidedeck.com/
 ...
 Version: 3.2.2
 Author: SlideDeck
 Author URI: http://www.slidedeck.com
 License: GPL3
 */
/*
...
class SlideDeckLitePlugin {
    ...
    static $st_friendly_name = "SlideDeck 3";
    ...
    function add_hooks( ) {
        ...
        // pages
        add_action( 'admin_print_scripts-slidedeck-3_page_' . SLIDEDECK2_HOOK . '/options', array( &$this, 'admin_print_scripts' ) );
        add_action( 'admin_print_styles-slidedeck-3_page_' . SLIDEDECK2_HOOK . '/options', array( &$this, 'admin_print_styles' ) );
        add_action( 'admin_print_scripts-slidedeck-3_page_' . SLIDEDECK2_HOOK . '/upgrades', array( &$this, 'admin_print_scripts' ) );
        add_action( 'admin_print_styles-slidedeck-3_page_' . SLIDEDECK2_HOOK . '/upgrades', array( &$this, 'admin_print_styles' ) );
        ...
        add_action( 'admin_print_scripts-slidedeck-3_page_' . SLIDEDECK2_HOOK . '/lenses', array( &$this, 'admin_print_scripts' ) );
        add_action( 'admin_print_styles-slidedeck-3_page_' . SLIDEDECK2_HOOK . '/lenses', array( &$this, 'admin_print_styles' ) );
   		add_action( 'admin_print_scripts-slidedeck-3_page_' . SLIDEDECK2_HOOK . '/addons', array( &$this, 'admin_print_scripts' ) );
        add_action( 'admin_print_styles-slidedeck-3_page_' . SLIDEDECK2_HOOK . '/addons', array( &$this, 'admin_print_styles' ) );
    ...
    function admin_menu( ) {
        ...
        if( $show_menu === true ) {
            add_menu_page( 'SlideDeck 3', 'SlideDeck 3', $this->roles['main_menu'], SLIDEDECK2_BASENAME, array( &$this, 'page_route' ), SLIDEDECK2_URLPATH . '/images/icon.png', 37 );
		...
		$this->menu['addon'] = add_submenu_page( SLIDEDECK2_BASENAME, 'SlideDeck Addon', 'Addons', $this->roles['manage_lenses_menu'], SLIDEDECK2_BASENAME . '/addons', array( &$this, 'page_addon_route' ) );
    ...
    function ajax_delete_addon( ) {
        ...
        if( !$response['error'] ) {
            ...
            if( $addon == false ) {
                ...
                slidedeck2_km( "SlideDeck Deleted Addon", array( 'slug' => $data['lens'] ) );
            }
        }
    ...
    function delete_addon_authorize( $lens, $redirect = '' ) {
        ...
        $custom_addon_dir = SLIDEDECK3_ADDONS_DIR;
        ...
        slidedeck2_km( "SlideDeck Deleted Addon", array( 'slug' => $lens ) );
    ...
    function discount_available_message() {
        if ( $this->is_plugin() && get_current_screen()->id != $this->menu['upgrades'] ) {
            ...
            if ( $days_left > 86400 ) {
                ...
                $message .= '<p><strong>Upgrade to SlideDeck 3 in the next ' . $days_left . ' days and get 25% off -</strong> Like what you\'ve seen with SlideDeck 3 Lite so far?. ';
            } else {
                $days_left = 0;
                $message .= '<p><strong>Last day to save 25% to upgrade to SlideDeck 3! -</strong> Like what you\'ve seen with SlideDeck 3 Lite so far?. ';
            }
            
            $message .= '<a href="' . $this->action( '/upgrades' ) . '">Upgrade to SlideDeck 3 Now</a>';
            $message .= '</p></div>';
    ...
    function page_addon_add( ) {
        ...
        include (SLIDEDECK2_DIRNAME . '/views/addons/add.php');
    }
    ...
    function page_addons_manage( ) {
        ...
        include (SLIDEDECK2_DIRNAME . '/views/addons/manage.php');
    }
...
/**
    ...
    function register_post_types( ) {
        register_post_type( 'slidedeck2', array( 'labels' => array( 'name' => 'slidedeck2', 'singular_name' => __( 'SlideDeck 3', $this->namespace ) ), 'public' => false ) );
    }
    ...
    function route( ) {
        ...
        if( preg_match( "/admin\.php\?.*page\=" . SLIDEDECK2_BASENAME . "\/support/", $uri ) ) {
            wp_redirect( "http://support.slidedeck.com/" );
            exit ;
        }
...
/**
	...
	function upload_addon( ) {
        ...
        if( !class_exists( 'SlideDeck_Addons_Upload' ) )
            include (SLIDEDECK2_DIRNAME . '/classes/slidedeck-addons-upload.php');
    ...
    /**
     * Load SlideDeck support CSS files for lenses used by SlideDecks on a pageadd_action( 'load-plugins.php',function(){
	...
	function slidedeck_gettext( $translated_text, $untranslated_text, $domain )
	{
		...
		$new = "Please delete <b>SlideDeck 2</b> first and then Install <b>SlideDeck 3</b>";
    ...
    function wp_register_scripts( ) {
        ...
        wp_register_script( "{$this->namespace}-library-js", SLIDEDECK2_URLPATH . '/js/slidedeck.jquery' . (SLIDEDECK2_ENVIRONMENT == 'development' ? '.dev' : '') . '.js', array( 'jquery' ), '1.3.9', true );
        // Public Javascript
        wp_register_script( "{$this->namespace}-public", SLIDEDECK2_URLPATH . '/js/slidedeck-public' . (SLIDEDECK2_ENVIRONMENT == 'development' ? '.dev' : '') . '.js', array( 'jquery', 'slidedeck-library-js' ), SLIDEDECK2_VERSION, true );
        // Mouse Scrollwheel jQuery event library
        wp_register_script( "scrolling-js", SLIDEDECK2_URLPATH . '/js/jquery-mousewheel/jquery.mousewheel.min.js', array( 'jquery' ), '3.0.6', true );
        // Fancy Form Elements jQuery library
        wp_register_script( "{$this->namespace}-fancy-form", SLIDEDECK2_URLPATH . '/js/fancy-form' . (SLIDEDECK2_ENVIRONMENT == 'development' ? '.dev' : '') . '.js', array( 'jquery' ), '1.0.0', true );
        // Tooltipper jQuery library
        wp_register_script( "tooltipper", SLIDEDECK2_URLPATH . '/js/tooltipper' . (SLIDEDECK2_ENVIRONMENT == 'development' ? '.dev' : '') . '.js', array( 'jquery' ), '1.0.1', true );
        // jQuery Easing Library
        wp_register_script( "jquery-easing", SLIDEDECK2_URLPATH . '/js/jquery.easing.1.3.js', array( 'jquery' ), '1.3', true );
        // jQuery MiniColors Color Picker
        wp_register_script( "jquery-minicolors", SLIDEDECK2_URLPATH . '/js/jquery-minicolors/jquery.minicolors.min.js', array( 'jquery' ), '7d21e3c363', true );
        // SlideDeck Preview Updater
        wp_register_script( "{$this->namespace}-preview", SLIDEDECK2_URLPATH . '/js/slidedeck-preview' . (SLIDEDECK2_ENVIRONMENT == 'development' ? '.dev' : '') . '.js', array( 'jquery' ), SLIDEDECK2_VERSION, true );
        // Simple Modal Library
        wp_register_script( "{$this->namespace}-simplemodal", SLIDEDECK2_URLPATH . '/js/simplemodal' . (SLIDEDECK2_ENVIRONMENT == 'development' ? '.dev' : '') . '.js', array( 'jquery' ), '1.0.1', true );
        // Twitter Intent API
        ...
        // jQuery Masonry
        wp_register_script( 'jquery-masonry', SLIDEDECK2_URLPATH . '/js/jquery.masonry.js', array( 'jquery' ), '2.1.01', true );
		// Jail library
		wp_register_script( 'jail', SLIDEDECK2_URLPATH . '/js/jail.js', array(), SLIDEDECK2_VERSION, true );
    }
...
add_action( 'plugins_loaded', array( 'SlideDeckLitePlugin', 'instance' ), 15 );
	...
	add_action( 'load-plugins.php',function(){
        add_filter( 'gettext', 'slidedeck_lite_gettext', 99, 3 );
    }
	);
	function slidedeck_lite_gettext( $translated_text, $untranslated_text, $domain )
	{
		$old = array(
			"Plugin <strong>activated</strong>.",
			"Selected plugins <strong>activated</strong>." 
		...
		$new = "Please delete <b>SlideDeck 2 Lite</b> first and then Install <b>SlideDeck 3 Lite</b>";
